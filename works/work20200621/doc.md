...
<img src="./img/cs2a_error_features_boxplot2.png" width = "480px"> 
...
In this context of the control theory,
the regularization term, which is defined as the norm of the manipulation values,
often serves the restriction of the abrupt change of the action
and then leads to softer response against the disturbance.

The weight parameter, which is the proportional rate to combine the two components of the rewards:
the absolute value of the error and the one of the action, can work as the regularization parameter.
Here, we study how the weight parameter affects the responsiveness against the disturbance
to design the controller properly.
...
where with the value closer to 1 the regularization gets stronger.

The learning curves of the trained parameters of policy  are shown in the figure 4.3.1(a),(b) and (c).
We say that with the stronger priority on the error
- the learning gets unstable, actually the derivative gain prioritized on the error the most is not significantly negative,
- the magnitudes of the proportinal and derivative gains become larger, which means that the controller can reponse against the disturbance more quickly,
- and the standard deviations of the noise generated by agents get larger because of the smaller regularization.
...
| gamma | reward discount factor | 0.9 |
| nSeq | the length of series to calculate derivative | 5 |
| weightOnError | controls the priority against the regularization term  | 0.5, 0.9 and 0.99 |
| agentEnableDcomponent | control the type of controller between P- or PD-controller | True(PD-controller only) |

<img src="./img/cs3a_p_gainD_distribution.png" width = "100%"> 
Figure 4.3.1(a): the learning curve of the trained derivative gains

<img src="./img/cs3a_p_gainP_distribution.png" width = "100%"> 
Figure 4.3.1(b): the learning curve of the trained proportional gains

<img src="./img/cs3a_policy_sd_distribution.png" width = "100%"> 
